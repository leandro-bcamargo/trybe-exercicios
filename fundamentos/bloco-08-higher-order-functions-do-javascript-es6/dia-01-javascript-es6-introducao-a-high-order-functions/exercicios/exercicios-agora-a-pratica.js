...
const sorteio = (betNumber, compare) => {
  let randomNumber = Math.floor((Math.random() * 5) + 1);
  return compare(randomNumber, betNumber);
}


const compare = (randomNumber, betNumber) => {
...
const answer = (correctAnswers, providedAnswers, compareAnswers) => compareAnswers(correctAnswers, providedAnswers);


const compareAnswers = (correctAnswers, providedAnswers) => {
  let score = 0;
  for (let i = 0; i < correctAnswers.length; i += 1) {
    if (providedAnswers[i] !== "N.A") {
...
console.log(answer(RIGHT_ANSWERS, STUDENT_ANSWERS, compareAnswers))